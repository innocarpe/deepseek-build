...
- **Vendored Grok builds run one at a time.** Each worktree has its own
  ...
  When sessions share the warm target the usual way
  (`CARGO_TARGET_DIR=<primary checkout>/third_party/grok-build/target`), that
  serialization is cargo's advisory flock on `target/debug/.cargo-lock` — a
  second build waits with no output, so the TUI shows a command that looks
  hung. `./scripts/vendor-build.sh status` reads that queue (holder, waiters,
  elapsed, worktree; exit 1 while busy), and `./scripts/vendor-build.sh clone
  <slug>` copies the warm target copy-on-write into
  `~/.cache/dsb-vendor-targets/<slug>` so the wait can be skipped (registry
  dependencies stay fresh, workspace crates rebuild; `prune` frees idle
  clones). Calling cargo from inside a cargo build or test on that shared
  target **is** a real deadlock; nothing under `third_party/grok-build` does
  this today.
