...
| `crates/codegen/xai-grok-pager/src/scrollback/blocks/user.rs` | `collapsed_max_lines(width, mode)` replaces the fixed `COLLAPSED_MAX_LINES` budget: at or below `COLLAPSED_NARROW_TERMINAL_COLS` (60) a collapsed prompt folds to `COLLAPSED_NARROW_MAX_LINES` (2); wider keeps 3; `Expanded` still never folds. The off-screen estimate asks `collapsed_row_budget` for that same count | The measured iPhone Orca pane is 55 columns. One row cut most of the submitted prompt; two rows keep enough of it to read, and the band has no vertical padding so the extra row is one line. Desktop widths (80+) keep today's three-line budget |
| `crates/codegen/xai-grok-pager/src/app/mouse.rs`, `app/agent_view/selection.rs`, `scrollback/state/selection.rs` | At or below `COLLAPSED_NARROW_TERMINAL_COLS`, a same-cell tap on a collapsed foldable user prompt expands it and pins that entry to the top of the scrollback with follow mode off. A later tap on any row of that opened echo folds it back to two lines, including a second tap inside the double-click window. A drag stays a text selection. `word_select` does not swallow either tap. Wider panes keep double-click fold and do not expand on a single click | A two-line phone echo is the prompt a touch pane can show, and one tap is the gesture that pane delivers. The second tap was counted as a desktop double-click and only the first line folded, so the echo opened and stayed open. A drag stays a selection, and wider panes keep double-click fold |
...
| `crates/codegen/xai-grok-pager/src/views/prompt_widget/mod.rs` | The bottom info line's rect is inset one cell per corner (`area.x + 1`, `area.width - 2`) instead of starting at `content_area.x` | Wide panes keep a blank pad before `╯`, so a long label cannot paint the divider rule. A phone pane does not paint that label on the divider at all |
| `crates/codegen/xai-grok-pager/src/views/prompt_widget/mod.rs`, `views/phone_bottom_band.rs`, `app/agent_view/render.rs` | At or below `COLLAPSED_NARROW_TERMINAL_COLS` (60) the prompt box's bottom divider is a plain `╰──╯` rule and `info_block` stays one row. The DeepSeek status row under the box carries both clusters: balance plus `cN%` on the left, model and permission on the right (`V4.1 Flash (max) · always-approve`). A leading `DeepSeek ` is dropped; a trailing effort parenthesis is dropped only when the permission mode would otherwise be cut. The shortcut-hint row stays dropped (`shortcuts_height = 0`) in every state. Wide panes keep the label on the divider and `cache N%` on its own row | Two content rows under the box (`DeepSeek V4.1 Flash (max) · always-approve` and `$10.77  cache 94%`) spent a row of the measured 55-column iPhone pane on chrome. One row gives that line back to the scrollback. Desktop widths keep the hint row, so the split layout stays |
...
| `crates/codegen/xai-grok-pager-render/src/appearance/config.rs`, `crates/codegen/xai-grok-pager/src/{views/{prompt_widget/mod.rs,agent.rs,queue_pane.rs},app/agent_view/render.rs}`, `crates/codegen/xai-grok-pager/docs/user-guide/{05-configuration.md,06-theming.md}` | The flush frame's pads become symmetric gutters: `block_pad_left 1`, `block_pad_right 2` (the right pad mirrors the accent rail's column on top of the left pad, so the text band sits 3 columns from each pane edge; `default_layout_gives_the_text_band_symmetric_gutters` pins the relation). The composer's text is inset one cell on all four sides — `BOX_PAD = MIN_HPAD + BOX_TEXT_INSET` for `eff_box_pad_*` and `vpad_top`, `info_block` carries the bottom inset row plus the divider (and the label row when narrow) — and `eff_box_pad_*` no longer follows `block_pad_*`. The turn status row drops its layout gap row, sits flush on the prompt (`TurnStatusConfig::gap` defaults false) and takes `ACCENT + block_pad_left` / `block_pad_right` insets like any text row; the queue pane's `content_area` uses the same left inset. `LayoutConfig::BOX_TEXT_INSET` is the one inset constant | 55x41 before/after: block text at column 2 → 3 with the right edge moving 53 → 51 (the rail's right side finally has its gutter, and both gutters measure 3); composer text sat on the borders on all four sides, now inset 1; the turn status row had a blank row above it and one below, now neither. 120x40 desktops move identically (the pads and insets are width-independent). Every scrollback text row — echo, thinking body, tool rows — and the prompt echo's timestamp now stop at the same right column |
...
| `crates/codegen/xai-grok-pager/src/scrollback/{blocks/user.rs,layout.rs,render.rs,wrappers/entry_renderer.rs}`, `scrollback/{scrollback_pane.rs,state/mod.rs}`, `views/{prompt_widget/mod.rs,welcome/prompt.rs,agent.rs}`, `app/agent_view/render.rs` | The prompt areas and every block measure their padding in the minimum unit. `UserPromptBlock::has_vpad_for_width` keeps the echo's pad rows at every width and `EntryRenderer::paint_narrow_pad_row` paints each one on a narrow pane as two eighths of a row (`▂` / `▆`, the dashboard halo's sub-cell split). `EntryChrome` (`entry_chrome`) gives each entry its own left/right chrome: the accent column for a rail or a band, the rail plus one column of air for a rail, none for a block with neither; `HorizontalLayout::new_with_chrome` and `block_content_width_for(entry, ..)` carry it through the pane's row layout, the wrap width and the paint. The composer's `vpad_top` is 1 (the top border, no text inset) with `info_block` back to the divider row alone, the timestamp overlay closes one column inside the entry's right edge for every message, and `BOTTOM_MARGIN_ROWS` is 0 so the bottom status row closes the frame | A pad row is ~2.15 columns of pitch at the phone's font: a whole row above or below the text is more white than the one-column inset the user approved, and 0 rows is less, so the pad is a fraction of a row. The report measured the echo's clock one column right of an agent message's, a rail glued to its body, text starting two columns in with no rail or band to justify it, and text wrapping two columns short of the frame |
...
PTY cases `tight_frame_and_phone_only_gates` (55x41 flush, text band at column 3
and its right edge three columns short of the pane edge, the arrow absent;
120x40 flush with the same insets and the arrow back) and
...
`phone_pane_keeps_a_plain_divider_and_no_label_row` (plus the width loop in
`phone_divider_keeps_the_pane_width_without_a_label_row` and
`phone_width_does_not_budget_a_second_info_row`) and end to end by
`app::agent_view::phone_bottom_tests`, which renders the whole view at 55×41
and asserts one band (cost left, model and permission right, including a large
balance, a long model and the other permission modes) plus the absent hint
row, and at 80×40, 120×40 and 180×50 that the desktop divider label and the
separate `cache N%` row are unchanged. `compose_phone_bottom_band` pins the
shrink ladder. The PTY case `bottom_band_is_one_row_on_the_phone_frame` checks
the same one-row shape on a live 55×41 frame and the divider label after a
resize to 80 and 180.
