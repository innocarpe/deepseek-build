...
| `crates/codegen/xai-grok-pager/src/scrollback/{blocks/user.rs,wrappers/entry_renderer.rs}`, `views/{prompt_widget/mod.rs,welcome/prompt.rs}`, `app/agent_view/render.rs` | The prompt areas keep the one-column inset their sides have: `UserPromptBlock::has_vpad_for_width` gates the echo's pad rows on `COLLAPSED_NARROW_TERMINAL_COLS` again (a phone pane keeps text rows only), the composer's `vpad_top` is 1 (the top border, no text inset row) with `info_block` back to the divider row alone, and the timestamp overlay closes one `block_pad_right` (floor 1) inside the entry's right edge | A pad row is ~2.15 columns of pitch at the phone's font, so one blank row above or below the text is more white than the one-column left/right inset the user approved; the echo's two pad rows also cost as much as its two-row band. The clock now stops inside the reserved 8-column gutter instead of closing on the band's last column |
