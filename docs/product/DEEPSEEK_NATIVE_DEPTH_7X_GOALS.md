# DeepSeek-native depth train — **`7.x`**

> [!NOTE]
> **PROPOSED.** This board has no execution authority until its PR merges and
> the owner accepts it. `main` currently carries `5.7.0`; the `6.x` base-refresh
> train owns the vendor tree until `6.0.0` lands.

| Field | Value |
|-------|--------|
| **Plan id** | `deepseek-native-depth-7x` |
| **SemVer path** | **`7.0.0`** (first cut). Minors as work lands. |
| **Depends on** | **`6.0.0` base refresh** for any unit touching `third_party/grok-build/`; wire-independent units may start earlier |
| **North star** | [VISION.md](./VISION.md) + [HARNESS_PHILOSOPHY.md](../architecture/HARNESS_PHILOSOPHY.md) |
| **PRD** | [PRD-v7.md](./PRD-v7.md) |
...
|---|---|---|---|
| **U0.1** | This evidence + plan: research doc, PRD-v7, board, index updates | none | — |
...
| **U1.2** | **Prompt/tool change path** — move the prompt so a change can append after cached history rather than rebuild the head; keep the epoch model as the fallback | U0.2 · **U2.1** if the wire investigation changes the answer | Design must state which behavior applies when the route cannot do in-history. |
...
|---|---|---|
| **U6.1** | Honesty table filled from merged results; owner-readable summary; release cut and publish | all claimed units |
...
2. **Vendor-touching units wait for `6.0.0`.** Units that only touch
   `crates/dsb-*` or docs run in parallel with the base refresh.
3. **One Grok build at a time across all worktrees** — this train's units are
...
5. **U5.x never blocks Waves 1–4.** If the wire answer changes the design of
   U1.2, U1.2 is revised in a follow-up PR; the train does not stall.

---

## 3. Per-unit honesty requirements
...
- The `6.0.0` base refresh changes the request/assembly code U1.2 and U3.1
  build on, after those units are written.
- Two consecutive units cannot be measured, which would mean the measurement
  story is wrong rather than the units.

---

## 5. What this train deliberately does not do

| Not done | Why |
|---|---|
| Anthropic Messages migration | Investigation, not a commitment — §6 of [PRD-v7](./PRD-v7.md). |
...
| New TUI surfaces | This line is depth, not surface. |
