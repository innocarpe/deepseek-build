...
| `vendor-build.sh` | Read the shared vendored-Grok build target queue — `status` reports file-lock holders/waiters, elapsed, command, worktree, host free/swap/load5m and the compile-gate verdict (exit 1 while busy) — and work around it: `clone` makes an APFS copy-on-write personal target under `~/.cache/dsb-vendor-targets/<slug>` (retried with backoff; a per-file clonefile fallback skips files a live build removes), `run -- <cmd>` passes a free queue through and otherwise starts only through the compile gate with `CARGO_BUILD_JOBS=2`, `prune` frees idle ones (base target untouched) |
| `test-vendor-build.sh` | Hermetic test for `vendor-build.sh` (fixture target tree + python3 file-lock holders, fake rustc children, a fake cp for the retry/fallback paths; no cargo, no network, no `~/.cache` writes) |
