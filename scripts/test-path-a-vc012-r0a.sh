#!/usr/bin/env bash
# VC012 Path A R0A — public deepseek-build/dsb worktree dogfood + bare-session honesty.
#
# Proves (vision V3-WT / L3-WT-1/2 Path A bar):
#   1. Public CLI → agent_launch → product agent (no DEEPSEEK_BUILD_AGENT_BIN)
#   2. worktree-cli-surface: agent --help + worktree --help + worktree list --json
#   3. Dual CLI: dsb agent worktree list when dsb sits beside deepseek-build
#   4. worktree-opt-in-stamp: path_a_l3 worktree_product=opt_in + bare_dsb_session=single
#   5. worktree-headless-no-create: -p --worktree=NAME creates no git worktree
#
# Usage:
#   ./scripts/test-path-a-vc012-r0a.sh
#   ./scripts/test-path-a-vc012-r0a.sh --keep
#   ./scripts/test-path-a-vc012-r0a.sh --scenario worktree-cli-surface
...
# --- shared: hermetic home with agent copy ---
setup_hermetic_home() {
  local HOME_DIR="$1"
  local BASE_URL="${2:-}"
...
EOF
    ...
    cat >"${HOME_DIR}/config.toml" <<'EOF'
# Hermetic Path A VC012 R0A home (CLI-only scenarios).
...
repo=${REPO}"
  ...
  # Headless honesty: no new worktree path and no name leak.
...
FAILED=0
SCENARIOS=(worktree-cli-surface worktree-opt-in-stamp worktree-headless-no-create)
