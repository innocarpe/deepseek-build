...
///
/// Pairing is decided against a name snapshot taken before any deletion, so the count is independent of visit order.
/// Pair ages are also snapshotted before deletion, so temp files never lose sidecar-derived age when a sidecar is visited first.
