...
/// Resolve `path` the way `resolve_destination` does, for expectations that
/// must compare against a resolved path. On macOS `tempfile` hands out paths
/// under `/var`, which is a symlink to `/private/var`, so a raw expectation
/// fails even though the resolver is correct.
#[cfg(unix)]
fn resolved(path: &std::path::Path) -> std::path::PathBuf {
    // Canonicalize the deepest existing ancestor, then re-attach the tail, so
    // this works for paths whose leaf does not exist yet.
...
fn resolve_atomic_destination_follows_leaf_symlink() {
    ...
    assert_eq!(resolved(&target), dest);
...
fn resolve_refuses_chain_past_darwin_maxsymlinks() {
    ...
    // `ELOOP` is an unstable `ErrorKind` and surfaces as `InvalidInput` on the
    // pinned toolchain, so match on the error rather than the kind: the
    // invariant under test is that a chain past the kernel's limit is refused.
    assert!(
        // ELOOP is 62 on Darwin and Linux; matching the raw code keeps this
        // independent of the unstable `ErrorKind::FilesystemLoop`.
        err.raw_os_error() == Some(62) || err.kind() == std::io::ErrorKind::InvalidInput,
        "expected an ELOOP-shaped refusal, got {err:?}"
    );
    assert!(
        std::fs::symlink_metadata(&head)
            .expect("head")
