...
/// Falls back to `DeepSeekNight` for an out-of-range byte, which `set` can't produce but a future variant missing from this match could.
fn theme_kind_from_u8(byte: u8) -> ThemeKind {
    match byte {
        x if x == ThemeKind::DeepSeekNightV2 as u8 => ThemeKind::DeepSeekNightV2,
