...
The DeepSeek Build picker exposes seven built-in themes, plus an `auto` option that follows your system appearance. Classic `deepseeknight` is the product/runtime/config default; `deepseeknight-v2` remains selectable as the first picker alternate:
