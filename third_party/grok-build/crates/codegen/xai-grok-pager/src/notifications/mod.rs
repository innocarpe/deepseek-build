...
impl NotificationService {
    ...
    /// Reset the tab title back to the product name and clear the progress bar so neither lingers after exit. Enqueued, never
...
mod tests {
    ...
    fn shutdown_enqueues_resets_on_the_writer_queue() {
        ...
        assert!(
            String::from_utf8_lossy(payload.data()).contains("DeepSeek Build"),
