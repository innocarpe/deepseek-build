...
fn theme_picker_offers_the_product_default() {
    ...
    assert!(
        ...
        "DeepSeek Night v2 must remain selectable in the picker"
    );
    assert!(
        choices.iter().any(|c| c.canonical == "deepseeknight"),
        "the classic product default must remain selectable in the picker"
    );
}
