...
pub fn extract_builtin_files(grok_home: &std::path::Path) {
    ...
    // Clean up the cached JSON changelog from a previous version so /release-notes fetches fresh structured entries for the new version.
    // The markdown cache ($GROK_HOME/CHANGELOG.md) is deliberately NOT deleted: DeepSeek Build seeds it with the
    // product changelog at every launch (dsb-cli `seed_product_changelog`), and removing it here made the
    // welcome-screen CHANGELOG click silently no-op after a version bump (the seed is written before the agent
    // starts, then deleted by this cleanup on the version-transition first run).
