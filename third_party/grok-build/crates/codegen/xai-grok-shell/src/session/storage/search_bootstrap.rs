...
#[derive(Default)]
pub(super) struct BootstrapProgress {
...
pub(super) struct BootstrappingGuard {
    progress: Arc<BootstrapProgress>,
    generation: u64,
}

impl BootstrappingGuard {
    pub(super) fn new(progress: &Arc<BootstrapProgress>) -> Self {
        Self {
...
pub(super) async fn try_bootstrap_with_lease(
    root_dir: &Path,
    storage: &dyn StorageAdapter,
    progress: &Arc<BootstrapProgress>,
) -> io::Result<BootstrapOutcome> {
...
async fn bootstrap_with_lease_inner(
    ...
    loop {
        ...
        if claimed {
            ...
            let result =
                reindex_all(root_dir, storage, progress, &token, refresher.claim_lost()).await;
            drop(refresher);
            release_bootstrap_claim(&db_path, &token).await;
            return result;
        }
...
#[cfg(test)]
pub(super) fn try_read_last_bootstrap_at(db_path: &Path) -> io::Result<Option<i64>> {
    if !db_path.exists() {
        return Ok(None);
    }
...
async fn reindex_all(
    ...
    // The StorageAdapter reference cannot be shared across tasks.
...
#[cfg(test)]
#[path = "search_bootstrap_tests.rs"]
mod tests;
