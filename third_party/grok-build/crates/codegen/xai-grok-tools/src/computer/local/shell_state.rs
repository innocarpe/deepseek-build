...
mod tests {
    ...
    ///
    /// NOTE: ignored on CI (ubuntu bash 5.2) — the wrapped `set -a` returns exit 126 there while passing
    /// on macOS (the product's target platform). The ubuntu-bash environment quirk needs a Linux repro
    /// to pin down; tracked as a follow-up. See the env-gated skips above for the pattern.
    #[tokio::test]
