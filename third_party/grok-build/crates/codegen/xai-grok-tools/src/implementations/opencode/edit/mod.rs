...
impl From<EditInput> for crate::types::tool_io::ToolInput {
    fn from(value: EditInput) -> Self {
        crate::implementations::grok_build::search_replace::SearchReplaceInput {
            ...
            // The opencode/Codex surface predates the session snippet table, so
            // it carries no version anchor: this is the same compatibility path
            // as a hand-written edit, not a snippet-safe Path A edit.
            file_version: None,
            snippet_id: None,
        }
        .into()
