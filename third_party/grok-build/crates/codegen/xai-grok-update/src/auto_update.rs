...
fn reinstall_hint(installer: &str, _channel: &str) -> String {
    match installer {
        ...
        // The product ships only via npm / GitHub Releases and has no x.ai
        // bootstrap installer, so an unclassified install still reinstalls the
        // npm package rather than curl-ing an upstream script.
...
pub fn print_update_status(status: &UpdateStatus, json: bool) -> anyhow::Result<()> {
    ...
    if status.update_available {
        if let Some(latest_version) = status.latest_version.as_deref() {
            println!(
                "A new version of DeepSeek Build is available: {} -> {}{}",
                status.current_version, latest_version, channel_label
            );
        } else {
            println!("A new version of DeepSeek Build is available.");
        }
...
/// The DeepSeek Build product never auto-downgrades, regardless of how the install is classified.
/// Pointer rollback is an upstream Grok / x.ai-managed install concept (x.ai CDN pointers can be rolled back);
/// DeepSeek Build has no such pointer, and a "newer" version advertised from any source that is actually older
/// (e.g. the Grok Build channel) must never be installed over the product.
/// The signature is kept so the decision stays explicit at every call site.
