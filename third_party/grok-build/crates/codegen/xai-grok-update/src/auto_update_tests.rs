...
fn test_reinstall_hint_npm_mentions_npm_command() {
    ...
    assert!(
        ...
        "should name the product package: {hint}"
    );
}
...
fn test_reinstall_hint_gh_release_mentions_gh_command() {
    ...
    assert!(
        hint.contains("innocarpe/deepseek-build"),
        "should name the product repo: {hint}"
    );
}

#[test]
fn test_reinstall_hint_internal_mentions_product_npm() {
    // The product has no x.ai bootstrap installer: an "internal"-classified
    // install still reinstalls the product npm package.
    let hint = reinstall_hint("internal", "stable");
    assert!(
        hint.contains("@innocarpe/deepseek-build"),
        "product reinstall should point at the npm package: {hint}"
    );
    assert!(
...
fn test_reinstall_hint_ignores_channel_for_the_product() {
    ...
    for channel in ["stable", "alpha", "enterprise", "al pha", "x'; rm -rf ~;'", ""] {
        ...
        assert!(
            hint.contains("@innocarpe/deepseek-build"),
            "product reinstall must still name the npm package: {hint}"
        );
    }
}
...
fn test_installer_allows_downgrade_internal_blocked() {
    // The product has no x.ai pointer to roll back to; a lower version from any
    // source must never be installed over the product.
